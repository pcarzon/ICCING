...
ProgramDir=/projects/jnorhos/pcarzon/TrentoGluonSpectrum/build_UIUC/src
ShellDir=/projects/jnorhos/pcarzon/ICCING
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
		do
			...
			sed -e "s|PDIR|$ProgramDir|" -e "s|DDIR|$DataDir/"$i"|" -e "s|CONFFILE|$ShellDir/TrentoParameters"$i".conf|" -e "s|DTYPE|$DataType|" -e "s|NUM|$i|" submitTrento.sh > "$DataFile"_"$i".sh;
			#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
			...
			ICCINGID=$(sbatch -p qgp -A qgp --export=All,ConfigFile="$ShellDir/ICCINGParameters_$i.conf" --dependency=afterany:${TrentoID##* } ./submitICCING.sh);	
...
#???????????????????????????????????????????????????????????????????????????????????????????????????????????????
#	USE THIS TO COMBINE FILES: KEYWORD = Comb  (MOSTLY WORKS)
#???????????????????????????????????????????????????????????????????????????????????????????????????????????????
