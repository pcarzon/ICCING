...
namespace GreensFunctions{
    ...
    gsl_interp_accel **FsswTAcc,**FsvwTAcc;
    gsl_interp_accel **FssdXdTAcc,**FsvdXdTAcc;

    gsl_spline2d *FssInt,*FsvInt;
    ...
    double FsvScalingCurve(double wT,double dXdT){
        int tID=omp_get_thread_num();
        EVALUATE_GSL_INTERPOLATOR_2D(FsvInt,wT,dXdT,FsswTAcc[tID],FsvdXdTAcc[tID],wTMin,wTMax,dXdTMin,dXdTMax);
    } // FsvScalingCurve
    ...
    // READ INPUT FILE //
    // INPUT FILE MUST HAVE FOLLOWING STRUCTURE: 1:wTilde 2:|x-x_0|/|tau-tau_0| 3:|tau-tau_0|^2*Fss 4:|tau-tau_0|^2*Fsv //
    ...
    void SetValues(std::string fname,int NumberOfTimes,int NumberOfPoints){
        ...
        while(InStream.good()){
            ...
            double wT; double dXdT; double Fss; double Fsv;
                
            InStream >> wT; InStream >> dXdT; InStream >> Fss; InStream >> Fsv;
            ...
            FssVal[wCounter]=Fss; FsvVal[wCounter]=Fsv;
    ...
    void Output(std::string fname,int NwT,int NdXdT){
        ...
        Outstream << "# 1:wTilde=tau T(tau)/(eta/s) 2:|x-x_0|/|tau-tau0| 3:|tau-tau0|^2Fss 4:|tau-tau0|^2Fsv" << std::endl;
        ...
        for(int tIndex=0;tIndex<NwT;tIndex++){
            ...
            for(int xIndex=0;xIndex<NdXdT;xIndex++){
                ...
                Outstream << wTi << " " << dXdTi << " " << Fss(wTi,dXdTi) << " " << Fsv(wTi,dXdTi) << std::endl;
